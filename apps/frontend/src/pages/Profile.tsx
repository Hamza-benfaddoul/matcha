...
function Profile() {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          `/api/images/update-image/profile/${user.id}`,
          formData,
          {
            headers: {
  ...
  const isLikedFunc = async () => {
    try {
      const response = await axios.get(`/api/user/likes/isLiked/`, {
        params: { likedId: id },
      });
  ...
  const isBlockedFunc = async () => {
    const reponse = await axios.get(`/api/user/block/isBlocked/`, {
      params: { blocker_id: auth.user.id, blocked_id: id },
    });
  ...
  useEffect(() => {
    ...
    if (id != auth.user.id) {
      const viewResponse = axios.post(`/api/user/views/add-view`, {
        viewedId: id,
      });
    }
    fetchCountViews();
  ...
  return (
    <div className="flex h-full w-full justify-center">
      <div
        className={`flex flex-col md:w-[calc(100%-150px)] w-full ${isBlocked ? "h-full justify-center items-center" : ""}`}
      >
        {isBlocked ? (
          <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full text-center">
            ...
            <p className="text-gray-600 mb-8">
              You've blocked this user. You can't view their profile or interact
              with their content.
            </p>
            ...
            <p className="text-sm text-gray-500 mt-6">
              You can manage all blocked users in your{" "}
              <a href="/blocks" className="text-blue-500 hover:underline">
                block list
              </a>
              .
            </p>
                  ...
                  <div className="flex items-center text-[17px] text-gray-500">
                    {!isMyProfile && (
                      <>
                        ...
                        {isOnline
                          ? "Online"
                          : `Offline - ${formatLastLogin(user.last_login)}`}
                      </>
                    )}
                  </div>
                      ...
                      isLiked ? (
                        <div className="flex items-center px-4 py-2">
                          <button
                            ...
                            className="bg-[#F02C56] text-white px-4 py-2 rounded-md mr-2"
                          >
                            Unlike
                          </button>
                          {isConnected ? (
                    ...
                    {isMyProfile && (
                      <div>
                        <div className="flex gap-2">
                          <button
                            onClick={openModal}
                            className="bg-[#ff6b4e] text-white px-4 rounded-md"
                          >
                            {" "}
                            Edit Profile
                          </button>
