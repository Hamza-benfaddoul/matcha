...
function Chat() {
  ...
  const handleSelectChat = async (contact) => {
    ...
    const chatHistory = await axios.get(`/api/user/chat/get-messages`, {
      params: { userId: currentUser.id, contactId: contact.id },
    });
    setTimeout(() => {
      const mockMessages = [
        {
  ...
  return (
    <div className="flex h-full bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      {/* Chat window */}
      <div
        className={`${isMobileView ? (showSidebar ? "hidden" : "flex") : "flex"} flex-1 flex-col`}
      >
        {activeChat ? (
          <ChatWindow
            currentUser={currentUser}
